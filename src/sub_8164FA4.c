...
s32 sub_80184F4(void *);                        /* extern */

void sub_81650DC(void *arg0) {
    sub_80184F4((*(void **)((s8 *)(arg0) + (0x38))));
    sub_8018218((*(void **)((s8 *)(arg0) + (0x20))), (*(void **)((s8 *)(arg0) + (0x38))), 0x3000U, 0x20, 0);
}

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8165102.s\"");
#else
...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_816512C.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_81651D4.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_816521C.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
