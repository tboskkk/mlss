#include "global.h"
#include "common.h"

// TODO: add other includes (a matching *.h, other subsystem headers) as
// this file grows. Left minimal since split_func.py doesn't know yet what
// sub_80B30D8 needs.

asm_unified(".include \"asm/macros.inc\"");

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B30D8.s\"");
#else
#error "TODO: write sub_80B30D8 to match asm/nonmatching/sub_80B30D8.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B3170.s\"");
#else
#error "TODO: write sub_80B3170 to match asm/nonmatching/sub_80B3170.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B31F0.s\"");
#else
#error "TODO: write sub_80B31F0 to match asm/nonmatching/sub_80B31F0.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B325C.s\"");
#else
#error "TODO: write sub_80B325C to match asm/nonmatching/sub_80B325C.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B328C.s\"");
#else
#error "TODO: write sub_80B328C to match asm/nonmatching/sub_80B328C.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B32FC.s\"");
#else
#error "TODO: write sub_80B32FC to match asm/nonmatching/sub_80B32FC.s, then delete this #error"
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_80B3390.s\"");
#else
#error "TODO: write sub_80B3390 to match asm/nonmatching/sub_80B3390.s, then delete this #error"
#endif
