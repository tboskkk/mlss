#include "global.h"
#include "common.h"

// TODO: add other includes (a matching *.h, other subsystem headers) as
// this file grows. Left minimal since split_func.py doesn't know yet what
// sub_8114790 needs.

asm_unified(".include \"asm/macros.inc\"");

void sub_8114790(struct Entity *arg0) {
    s32 temp_r2_11;

    temp_r2_11 = 0x38 & (*(u8 *)((s8 *)(arg0) + (0x76)));
    if (temp_r2_11 == 0) {
        (*(s32 *)((s8 *)(arg0) + (0x4C))) = temp_r2_11;
        (*(s8 *)((s8 *)((*(void **)((s8 *)(arg0->unk2C) + (0x28)))) + (0x113))) = (s8) temp_r2_11;
    }
}

s32 sub_811458C();                              /* extern */

void sub_81147B4(void *arg0) {
    sub_811458C();
    (*(s8 *)((s8 *)((*(void **)((s8 *)(arg0) + (0x28)))) + (0x113))) = 0;
}

ASM_FUNC("asm/nonmatching/sub_81147D0.s", void sub_81147D0(void *arg0));
s32 sub_8082E1C(void *, s32, s32, s32);         /* extern */

void sub_8114854(struct Entity *arg0) {
    void *temp_r1_15;

    if (8 & (*(u8 *)((s8 *)(arg0->unk08) + (0x12)))) {
        temp_r1_15 = *(void **)0x03000FD8;
        (*(s32 *)((s8 *)((*(void **)((s8 *)(temp_r1_15) + (0x60)))) + (0x54))) = 0;
        (*(s32 *)((s8 *)((*(void **)((s8 *)(temp_r1_15) + (0x64)))) + (0x54))) = 0;
        sub_8082E1C(arg0, -1, -1, 0);
        (*(s32 *)((s8 *)(arg0) + (0x4C))) = 0;
    }
}

ASM_FUNC("asm/nonmatching/sub_811488C.s", void sub_811488C(void *arg0));
ASM_FUNC("asm/nonmatching/sub_81148B8.s", void sub_81148B8(void *arg0));
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8114A80.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8114A84.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8114AB8.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8114B30.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

ASM_FUNC("asm/nonmatching/init_ybpa_process_8114B4C.s", void * init_ybpa_process_8114B4C(void *arg0, u8 arg1, u8 arg3));