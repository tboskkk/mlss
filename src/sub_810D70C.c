#include "global.h"
#include "common.h"

// TODO: add other includes (a matching *.h, other subsystem headers) as
// this file grows. Left minimal since split_func.py doesn't know yet what
// sub_810D70C needs.

asm_unified(".include \"asm/macros.inc\"");





s32 stop_sfx_80195A8();
void sub_810FE44(void *arg0);

u8 sub_8087CE4();
void sub_810FE1C(struct Entity *arg0);

s32 sub_80883A0();
s32 sub_8082E1C();
void sub_810D70C(void *arg0)
{
  int new_var4;
  s16 *new_var2;
  void *new_var;
  s32 *new_var3;
  new_var = (void *) 0x0400001A;
  new_var4 = (*((u16 *) 0x0200001A)) + (*((u16 *) (((s8 *) arg0) + 0x14)));
  *((s16 *) (((s8 *) ((void *) 0x0400001A)) + 0)) = (s16) new_var4;
  new_var2 = (s16 *) (((s8 *) new_var) + 4);
  new_var3 = (s32 *) (new_var = ((s8 *) arg0) + 4);
  *new_var2 = (s16) ((*((u16 *) (((s8 *) arg0) + 0x14))) + (*((u16 *) 0x0200001E)));
  *new_var3 = 0;
}

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810D738.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810DD7C.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810DEB8.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810E03C.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810E360.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810E5D0.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F5E4.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F6D4.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F764.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F824.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F904.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810F9FC.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FA8C.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FB20.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FBC4.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

void sub_810FBFC(void *arg0)
{
  void *temp_r2_17;
  if ((sub_8087CE4() << 0x18) == 0)
  {
    sub_8082E1C(arg0, 8, 0, 0);
    temp_r2_17 = *((void **) (((s8 *) arg0) + 8));
    *((u8 *) (((s8 *) temp_r2_17) + 0x12)) = (u8) (((-7) & (*(((s8 *) temp_r2_17) + 0x12))) | 2);
    *((s32 **) (((s8 *) arg0) + 0x4C)) = (s32 *) (&sub_810FE1C);
  }
}

void sub_810FC34(void *arg0)
{
  void *temp_r2_12;
  if ((sub_8087CE4() << 0x18) == 0)
  {
    temp_r2_12 = *((void **) (((s8 *) arg0) + 8));
    *((u8 *) (((s8 *) temp_r2_12) + 0x12)) = (u8) (((-7) & (*(((s8 *) temp_r2_12) + 0x12))) | 2);
    sub_80883A0(arg0, 0x28);
    stop_sfx_80195A8(0x112);
    *((s32 **) (((s8 *) arg0) + 0x4C)) = (s32 *) (&sub_810FE44);
  }
}

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FC70.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

s32 stop_sfx_80195A8(s32);                      /* extern */
u8 sub_8087CE4();                                   /* extern */
void sub_810FCE0(void *arg0)
{
  char temp_r5_10;
  temp_r5_10 = sub_8087CE4();
  if (temp_r5_10 == 0)
  {
 do { sub_8082E1C(arg0, -1, -1, 0); stop_sfx_80195A8(0x11B); } while (0);
    *((s32 *) (((s8 *) arg0) + 0x4C)) = (s32) temp_r5_10;
  }
}

extern s32 sub_810F824;

void sub_810FD10(struct Entity *arg0) {
    void *temp_r2_8;

    temp_r2_8 = (*(void **)((s8 *)(arg0) + (0x30)));
    if (8 & (*(u8 *)((s8 *)((*(void **)((s8 *)(temp_r2_8) + (8)))) + (0x12)))) {
        sub_8082E1C(temp_r2_8, 0, 0, 0);
        sub_80883A0(arg0, 0xC);
        play_sfx_80195B4(0x10F, -1);
        arg0->handler = &sub_810F824;
    }
}

extern s32 sub_810FEF4;
void sub_810FD54(void *arg0)
{
  void *temp_r2_17;
  int new_var;
  new_var = -7;
  if ((sub_8087CE4() << 0x18) == 0)
  {
    sub_8082E1C(arg0, 0xF, 0, 0);
    temp_r2_17 = *((void **) (((s8 *) arg0) + 8));
    *((u8 *) (((s8 *) temp_r2_17) + 0x12)) = (u8) ((new_var & (*((u8 *) (((s8 *) temp_r2_17) + 0x12)))) | 2);
    stop_sfx_80195A8(0x112);
    *((s32 **) (((s8 *) arg0) + 0x4C)) = &sub_810FEF4;
  }
}

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FD94.s\"");
#else
/* Undraftable by m2c: its seed did not compile. Deliberately left empty so
   the REST of this translation unit still builds and can be diffed under
   NONMATCHING=1. The #ifndef branch above is unaffected -- the real ROM
   still gets the verbatim retail bytes. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FE04.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_810FE10.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
   the real ROM still gets the verbatim retail bytes and progress.py still
   counts this as unmatched. Write the C here, replacing this comment. */
#endif

void sub_810FE1C(struct Entity *arg0) {
    if (8 & (*(u8 *)((s8 *)(arg0->unk08) + (0x12)))) {
        sub_8082E1C(arg0, -1, -1, 0);
        (*(s32 *)((s8 *)(arg0) + (0x4C))) = 0;
    }
}

s32 sub_8082E1C(void *, s32, s32, s32);         /* extern */
s32 sub_80883F0(void *);                            /* extern */
extern s32 sub_810FA8C;
void sub_810FE44(void *arg0)
{
  void *temp_r2_8;
  int new_var2;
  void *temp_r2_29;
  s8 *new_var;
  temp_r2_8 = *((void **) (((s8 *) arg0) + 8));
  new_var2 = -7;
  if ((*((u8 *) (((s8 *) (*((void **) (((s8 *) arg0) + 8)))) + 0x23))) == 1)
  {
    *((u8 *) (((s8 *) temp_r2_8) + 0x12)) = (u8) ((*((u8 *) (((s8 *) (*((void **) (((s8 *) arg0) + 8)))) + 0x12))) | 0x10);
  }
  if ((sub_80883F0(arg0) << 0x18) == 0)
  {
    sub_8082E1C(arg0, 0xB, 0, 0);
    temp_r2_29 = *((void **) (((s8 *) arg0) + 8));
    *((u8 *) (((s8 *) temp_r2_29) + 0x12)) = (u8) ((new_var2 & (*((u8 *) (((s8 *) temp_r2_29) + 0x12)))) | 2);
    new_var = ((s8 *) arg0) + 0x4C;
    *((s32 **) new_var) = &sub_810FA8C;
  }
}

s32 sub_8082E1C(void *, s32, s32, s32);         /* extern */
s32 sub_80883A0(void *, s32);                   /* extern */
extern s32 sub_810FF78;
void sub_810FE94(void *arg0)
{
  if (8 & (*((u8 *) (((s8 *) (*((void **) (((s8 *) arg0) + 8)))) + 0x12))))
  {
    sub_8082E1C(arg0, 0xE, 0, 0);
    sub_80883A0(arg0, 0x1E);
    *((s32 **) (((s8 *) arg0) + 0x4C)) = &sub_810FF78;
  }
}

s32 sub_8082E1C(void *, s32, s32, s32);         /* extern */
extern s32 sub_810FFB0;
void sub_810FEC8(void *arg0)
{
  int new_var;
  void *temp_r2_12;
  sub_8082E1C(arg0, 2, 0, 0);
  temp_r2_12 = *((void **) (((s8 *) arg0) + 8));
  new_var = *((u8 *) (((s8 *) temp_r2_12) + 0x12));
  new_var = ((-7) & new_var) | 2;
  *((u8 *) (((s8 *) temp_r2_12) + 0x12)) = (u8) new_var;
  *((s32 **) (((s8 *) arg0) + 0x4C)) = &sub_810FFB0;
}
