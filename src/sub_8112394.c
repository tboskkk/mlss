...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8112414.s\"");
#else
...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8112430.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
...
#ifndef NONMATCHING
asm_unified(".include \"asm/nonmatching/sub_8112444.s\"");
#else
/* No C attempt yet. Deliberately EMPTY rather than an #error: agbcc
   compiles a whole translation unit at a time, so an #error here fails
   every OTHER function in this file under NONMATCHING=1. Guard intact, so
...
void sub_8112450(void *arg0)
{
  ...
  if ((temp_r0_11 << 0x10) == 0)
  {
    ...
    new_var = &(*((void **) (new_var2 = ((s8 *) arg0) + 8)));
    temp_r2_21 = *new_var;
